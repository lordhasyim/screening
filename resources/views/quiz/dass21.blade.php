...
<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            ...
            <!-- Information Card (Show only on first load) -->
...
$(document).ready(function() {
    ...
    function selectAnswer($option) {
        ...
        // Track high-risk responses (for sensitive questions)
        trackQuestionResponse(value);
        
        // Auto-advance after 1.5 seconds (slightly longer than PHQ-9 for DASS-21)
        setTimeout(() => {
    ...
    function trackQuestionResponse(value) {
        // Track high-risk responses for sensitive questions (self-harm, suicidal ideation, etc.)
        const sensitiveQuestions = [18, 19, 29]; // Adjust indices based on your questions
    ...
    function getCompletionTime() {
        const startTime = sessionStorage.getItem('dass21_start_time');
        if (startTime) {
    ...
    // Track start time when quiz actually starts
    function trackStartTime() {
        if (!sessionStorage.getItem('dass21_start_time')) {
            sessionStorage.setItem('dass21_start_time', Date.now().toString());
        }
    }
    ...
    $('#startBtn').on('click', trackStartTime);
});

// Add required CSS (same as PHQ-9)
$('<style>')
.prop('type', 'text/css')
.html(`
    .answer-option {
